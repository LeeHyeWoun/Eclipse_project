...
public class MemberMgr {
	...
	public boolean deleteMember(String id) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				String deleteSql = "delete from tblMember where id=?";
	...
	// 모든회원정보 리턴
	public MemberBean getAllMember(String id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = pool.getConnection();
			sql = "select * from tblMember where and id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(id) from tblMember where id!='admin'";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(id) from  tblMember where id!='admin' and " + keyField + " like ? ";
				pstmt = con.prepareStatement(sql);
	...
	// 게시판 리스트
	public Vector<MemberBean> getMemberList(String keyField, String keyWord, int start, int end) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = pool.getConnection();
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select * from tblMember where id!='admin' order by id limit ?, ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			} else {
				sql = "select * from  tblMember where id!='admin' and " + keyField + " like ? ";
